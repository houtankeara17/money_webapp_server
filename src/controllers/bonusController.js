...
async function adjustSalaryForBonus(userId, year, monthNumber, deltaAmount, currency, user, meta = {}) {
  ...
  salary.amountUSD = toUSD(salary.amount, salary.currency || currency || "USD", user);
  await salary.save();
  ...
  try {
    await SalaryBonusHistory.create({
      ...
      bonusAmount: meta.bonusAmount != null ? Number(meta.bonusAmount) : Math.abs(delta),
      delta,
      noted: meta.noted || "",
    });
  } catch (err) {
...
const createBonus = asyncHandler(async (req, res) => {
  const { amount, currency, year, monthNumber, tag, status, noted, image, paymentMethod } =
    req.body;

  if (!amount || !currency || !year || !monthNumber || !tag) {
    return error(res, "Amount, currency, year, month and tag are required", 400);
  }
  ...
  return success(
    ...
    linked
      ? "Bonus added — linked salary updated"
...
const updateBonus = asyncHandler(async (req, res) => {
  ...
  if (!bonus) {
    return error(res, "Bonus not found", 404);
  }
  ...
  const fields = [
    "amount", "currency", "year", "monthNumber", "tag",
    "status", "noted", "image", "paymentMethod",
  ];
  fields.forEach((f) => {
  ...
  if (prevYear !== updated.year || prevMonth !== updated.monthNumber) {
    await adjustSalaryForBonus(req.user._id, prevYear, prevMonth, prevAmount, updated.currency, req.user, {
      action: "bonus_updated",
      bonusId: updated._id,
      bonusAmount: prevAmount,
      noted: "moved from previous month",
    });
    await adjustSalaryForBonus(req.user._id, updated.year, updated.monthNumber, -Number(updated.amount), updated.currency, req.user, {
      action: "bonus_updated",
      bonusId: updated._id,
    ...
    await adjustSalaryForBonus(req.user._id, updated.year, updated.monthNumber, delta, updated.currency, req.user, {
      action: "bonus_updated",
      bonusId: updated._id,
      bonusAmount: Number(updated.amount),
    });
  ...
  return success(res, updated, "Bonus updated — linked salary adjusted");
});
...
const deleteBonus = asyncHandler(async (req, res) => {
  ...
  return success(res, null, "Bonus deleted — salary restored");
});
...
const deleteAllBonuses = asyncHandler(async (req, res) => {
  ...
  return success(
    ...
    `Deleted ${result.deletedCount} bonuses`
  );
});

...
const PROVIDERS = [
  "ABA Bank", "ACLEDA Bank", "Wing",
  "Street Exchange", "Airport", "Other",
];
const CURRENCIES = ["USD", "KHR", "THB"];

const getExchangeLogs = asyncHandler(async (req, res) => {
  const {
    page = 1, limit = 20, year, monthNumber, provider,
    fromCurrency, toCurrency, sort = "-exchangeDate",
  } = req.query;
  ...
  const items = await ExchangeLog.find(query).sort(sort).skip(skip).limit(Number(limit));
...
const createExchangeLog = asyncHandler(async (req, res) => {
  const {
    ...
    rateUsed, officialRate, provider, providerNote,
    exchangeDate, noted, images,
  } = req.body;

  if (!fromCurrency || !fromAmount || !toCurrency || !toAmount || !rateUsed || !provider) {
    return error(res, "From/to currencies, amounts, rate and provider are required", 400);
  }

  if (fromCurrency === toCurrency) {
    return error(res, "From and to currencies must be different", 400);
  }
  ...
  return success(res, log, "Exchange log added successfully", 201);
});
...
const updateExchangeLog = asyncHandler(async (req, res) => {
  ...
  if (!log) return error(res, "Exchange log not found", 404);
  ...
  if (req.body.fromAmount !== undefined) log.fromAmount = Number(req.body.fromAmount);
  ...
  if (req.body.officialRate !== undefined && req.body.officialRate !== null && req.body.officialRate !== "") {
  ...
  return success(res, updated, "Exchange log updated successfully");
});
...
const deleteExchangeLog = asyncHandler(async (req, res) => {
  ...
  if (!log) return error(res, "Exchange log not found", 404);
  return success(res, null, "Exchange log deleted successfully");
});

const deleteAllExchangeLogs = asyncHandler(async (req, res) => {
  const result = await ExchangeLog.deleteMany({ userId: toObjectId(req.user._id) });
  return success(
    res,
    { deletedCount: result.deletedCount },
    `Deleted ${result.deletedCount} exchange logs`
  );
});

const exportExchangeLogs = asyncHandler(async (req, res) => {
  const items = await ExchangeLog.find({ userId: toObjectId(req.user._id) }).sort("-exchangeDate");
